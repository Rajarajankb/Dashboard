...
disease_columns = ['has_ari', 'has_diarrhea', 'convulsion', 'fever', 'jaundice', 
                    'vomiting', 'rash_boils', 'swollen_crusted_eyes']
...
sorted_diseases = sorted(total_counts.keys(), key=lambda x: total_counts[x], reverse=False)
...
fig2.add_trace(go.Bar(y=[disease_labels[disease] for disease in sorted_diseases], 
                     x=male_counts, 
                     orientation='h', 
                     name='Male', 
...
# Update layout
fig2.update_layout(title='Disease Prevalence by Gender',
...
# Create heatmap data
heatmap_data = pd.DataFrame(index=top_n_regions.index, columns=disease_columns)
for column in disease_columns:
    heatmap_data[column] = top_n_regions.index.map(disease_counts_by_region[column])
    
new_x_labels = ['Respiratory Infection', 'Diarrhea', 'Convulsion', 'Fever', 'Jaundice', 
                'Vomiting', 'Rash/Boils', 'Swollen/Crusted Eyes']
...
tab1, tab2 = st.tabs(['Child Health And Deaths', 'HBNC'])
...
with tab1:
    ...
    with col2:
        kpi("Infant Deaths", f"{infant_deaths_count} ")
    with col3:
        kpi("Neonatal Deaths", f"{neonatal_deaths_count}")
    with col4:
        kpi("Under-Five Deaths", f"{under_5_deaths_count}")
    ...
    with col2:
        st.markdown("#### Total Disease Count by Region (Heat map)")
...
with tab2:
    ...
    with col1:
        ...
        fig = go.Figure(data=[
            go.Bar(x=hbnc_types, y=hbnc_type_counts)
        ])

        # Update layout
        fig.update_layout(
            title="Number of Children who Completed Each HBNC Type",
            xaxis_title="HBNC Type",
            yaxis_title="Number of Children",
            barmode="stack"
        )
